...
def face_recognition():
    ...
    npy='face-recognition/npy'
    train_img="face-recognition/train_image"

    recognized_user = None  # Initialize the recognized_user variable
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            video_capture.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # set the width of the frame to 640 pixels
            video_capture.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # set the height of the frame to 480 pixels
            ...
            while True:
                                    ...
                                    if HumanNames[best_class_indices[0]] == H_i:
                                        ...
                                        print("Predictions: [ name: {}, accuracy: {:.3f} ]".format(result_names,
                                                                                                    best_class_probabilities[
                                                                                                        0]))
                                        cv2.rectangle(frame, (xmin, ymin - 20), (xmax, ymin - 2),
                                                      (0, 255, 255), -1)
                                        cv2.putText(frame, result_names, (xmin, ymin - 5),
                                                    cv2.FONT_HERSHEY_COMPLEX_SMALL,
                                                    1, (0, 0, 0), thickness=1, lineType=1)

                                        recognized_user = result_names  # Set recognized_user variable
                ...
                key = cv2.waitKey(1)
                if key == 113 or recognized_user is not None:  # "q" or user recognized
                    break
